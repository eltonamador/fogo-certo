-- ============================================================================
-- SISTEMA DE FREQUÊNCIA/CHAMADA - VERSÃO SEGURA
-- Remove objetos existentes antes de recriar
-- ============================================================================

-- Remover políticas RLS existentes
DROP POLICY IF EXISTS "Admins podem ver todas as aulas" ON public.aulas;
DROP POLICY IF EXISTS "Admins podem criar aulas" ON public.aulas;
DROP POLICY IF EXISTS "Admins podem editar todas as aulas" ON public.aulas;
DROP POLICY IF EXISTS "Instrutores veem suas aulas" ON public.aulas;
DROP POLICY IF EXISTS "Instrutores podem criar aulas" ON public.aulas;
DROP POLICY IF EXISTS "Instrutores podem editar suas aulas" ON public.aulas;
DROP POLICY IF EXISTS "Alunos veem aulas do pelotao" ON public.aulas;
DROP POLICY IF EXISTS "Admins podem ver todas as presencas" ON public.presencas;
DROP POLICY IF EXISTS "Instrutores gerenciam presencas de suas aulas" ON public.presencas;
DROP POLICY IF EXISTS "Alunos veem sua presenca" ON public.presencas;

-- Remover view
DROP VIEW IF EXISTS public.resumo_frequencia_aluno;

-- Remover tabelas (CASCADE remove as foreign keys)
DROP TABLE IF EXISTS public.presencas CASCADE;
DROP TABLE IF EXISTS public.aulas CASCADE;

-- Remover funções
DROP FUNCTION IF EXISTS public.atualizar_contadores_aula() CASCADE;
DROP FUNCTION IF EXISTS public.criar_presencas_pelotao(UUID, UUID);
DROP FUNCTION IF EXISTS public.publicar_chamada(UUID);
DROP FUNCTION IF EXISTS public.calcular_percentual_presenca(UUID, UUID, DATE, DATE);

-- Remover tipos ENUM (CASCADE remove dependências)
DROP TYPE IF EXISTS tipo_aula CASCADE;
DROP TYPE IF EXISTS status_presenca CASCADE;
DROP TYPE IF EXISTS status_aula CASCADE;
...
CREATE TABLE public.presencas (
  ...
  observacao TEXT,
  justificativa_url TEXT,
  justificativa_anexo TEXT,
...
CREATE POLICY "Instrutores podem deletar suas aulas rascunho"
  ON public.aulas FOR DELETE
  TO authenticated
  USING (
    instrutor_id = auth.uid() AND
