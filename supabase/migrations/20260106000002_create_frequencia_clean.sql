-- ============================================================================
-- SISTEMA DE FREQUÊNCIA/CHAMADA - INSTALAÇÃO LIMPA
-- ============================================================================

-- Remover tipos ENUM (CASCADE remove tudo: tabelas, funções, views, policies)
...
-- ============================================================================
-- FUNÇÃO: Criar presenças para pelotão
-- ============================================================================
...
CREATE POLICY "Instrutores veem suas aulas"
  ON public.aulas FOR SELECT TO authenticated
  USING (
    EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND (role = 'instrutor' OR role = 'admin'))
    AND (
...
CREATE POLICY "Instrutores podem deletar suas aulas rascunho"
  ON public.aulas FOR DELETE TO authenticated
  USING (
    instrutor_id = auth.uid() AND status = 'RASCUNHO' AND
    EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND (role = 'instrutor' OR role = 'admin'))
  );
...
CREATE POLICY "Alunos veem aulas do pelotao"
  ...
  USING (
    ...
    EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.pelotao_id = aulas.pelotao_id)
  );

-- PRESENCAS - Admins
CREATE POLICY "Admins podem gerenciar todas presencas"
  ON public.presencas FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role = 'admin'));

-- PRESENCAS - Instrutores
CREATE POLICY "Instrutores gerenciam presencas de suas aulas"
  ...
  USING (
    EXISTS (
      ...
      WHERE a.id = presencas.aula_id AND a.instrutor_id = auth.uid()
...
CREATE POLICY "Alunos veem sua presenca"
  ...
  USING (
    ...
    EXISTS (SELECT 1 FROM public.aulas a WHERE a.id = presencas.aula_id AND a.status = 'PUBLICADA')
  );
